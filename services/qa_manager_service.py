...
class QAManager:
    ...
    def find_match(self, question: str, language: str = "ar") -> dict | None:
        """
        ...
        for qa in self.qa_database["qa_pairs"]:
            ...
            if query:
                ...
                for lang in languages_to_search:
                    if lang in qa:
                        if query_lower in qa[lang]["question"].lower() or query_lower in qa[lang]["answer"].lower():
                            match_found = True
                            break
    ...
    def sync_languages(self, qa_id: str, base_language: str = "ar") -> bool:
        ...
        for qa in self.qa_database["qa_pairs"]:
            if qa["id"] == qa_id:
                _base_q = qa[base_language]["question"]
                _base_a = qa[base_language]["answer"]
        ...
        if format == "json":
            ...
            if "qa_pairs" in import_data:
                for qa in import_data["qa_pairs"]:
                    ...
                    exists = any(
                        existing["ar"]["question"] == qa["ar"]["question"] for existing in self.qa_database["qa_pairs"]
                    )
                    if not exists:
...
async def get_qa_response(question: str, language: str = "ar") -> str | None:
    ...
    if match_result:
        ...
        if requested_language in qa_pair:
            return cast(str | None, qa_pair[requested_language]["answer"])
        # Fallback to Arabic if language not found
        elif "ar" in qa_pair:
            return cast(str | None, qa_pair["ar"]["answer"])
