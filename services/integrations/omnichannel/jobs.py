...
async def handle_whatsapp_generate(job: QueueJob) -> dict[str, Any]:
    from services.integrations.whatsapp.ai_bridge import maybe_generate_and_send_ai_reply
...
async def handle_web_chat_generate(job: QueueJob) -> dict[str, Any]:
    from services.integrations.web_chat.generate_job import process_web_chat_generation_job
...
async def handle_tiktok_webhook_event(job: QueueJob) -> dict[str, Any]:
    from services.integrations.tiktok.webhook_jobs import process_claimed_webhook

    return await process_claimed_webhook(job.payload or {})


async def handle_operator_deliver(job: QueueJob) -> dict[str, Any]:
    from services.integrations.omnichannel.delivery_dispatch import deliver_outbox_row
