...
from services.integrations.omnichannel.dlq import mark_dead_letter, mark_needs_owner_action
from services.integrations.omnichannel.limiter import DistributedProviderLimiter
from services.integrations.omnichannel.metrics import incr
...
async def handle_omnichannel_deliver(job: QueueJob) -> dict[str, Any]:
    ...
    if snapshot.get("source") == "ai":
        from services.integrations.omnichannel.store import operator_takeover_blocks_ai
...
async def _send(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if channel == "whatsapp":
        from services.integrations.omnichannel.channel_whatsapp import deliver_whatsapp
    ...
    if channel == "web_chat":
        from services.integrations.omnichannel.channel_web_chat import deliver_web_chat

        return await deliver_web_chat(snapshot)
    from services.integrations.omnichannel.channel_meta import deliver_meta
