...
from services.integrations.web_chat.operation_lease import (
    assert_lease_fence,
    extend_lease,
    handoff_lease,
    lease_active,
...
from services.integrations.web_chat.operation_rows import get_operation_row, lease_ttl_seconds, row_to_record, utc_now
from services.integrations.web_chat.pg_models import WebChatOperationRow

# Compatibility aliases for sibling operation_* modules and tests.
_get_row = get_operation_row
_row_to_record = row_to_record
_now = utc_now
...
class WebChatOperationRepository:
    def claim(
        ...
        *,
        ...
        existing = get_operation_row(session, tenant_id=tenant_id, operation_key=operation_key, for_update=True)
        ...
        row = WebChatOperationRow(
            ...
            lease_generation=1,
            lease_expires_at=utc_now() + timedelta(seconds=lease_ttl_seconds()),
        ...
        try:
            ...
            session.rollback()
            raced = get_operation_row(session, tenant_id=tenant_id, operation_key=operation_key, for_update=True)
            if raced is None:
        ...
        return OperationClaimResult(status="created", record=row_to_record(row))
    ...
    def transition(
        ...
        *,
        ...
        row = get_operation_row(session, tenant_id=tenant_id, operation_key=operation_key, for_update=True)
        if row is None:
    ...
    def try_transition(
        ...
        *,
        ...
        if current != from_state:
            return row_to_record(row), False
        try:
            assert_lease_fence(row, lease_owner=lease_owner, lease_generation=lease_generation)
        ...
        if target == OperationState.RELEASED:
            row.reservation_id = None
        session.flush()
        return row_to_record(row), True

    def get(self, session: Session, *, tenant_id: str, operation_key: str) -> OperationRecord | None:
        row = get_operation_row(session, tenant_id=tenant_id, operation_key=operation_key)
...
from services.integrations.web_chat.operation_billing import (  # noqa: E402
