...
Added: offline case bank (≥800), IR metrics, claim verifier, contradiction amounts, injection sanitize, Arabic/Arabizi query fold, conversation resolve, readiness CLI (`python -m services.brain.readiness`), shadow compare helper (default off), lab feedback store, latency/load offline benches, index versioning helpers.
