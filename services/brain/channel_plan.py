...
def assert_channel_plan_allowed(tenant_id: str, channel: str) -> None:
    ...
    if "whatsapp" in name:
        from services.billing.membership.whatsapp_gate import assert_whatsapp_plan_allowed

        assert_whatsapp_plan_allowed(tenant_id)
        return
    if name.startswith("web") or name in {"website", "widget"}:
        from services.billing.membership.web_gate import assert_web_plan_allowed
