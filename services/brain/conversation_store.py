...
def reset_conversation_store_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.brain.conversation_store_pg import pg_reset, table_ready
...
def _sql_ready() -> bool:
    ...
    with optional_message_session() as session:
        if session is None:
            return False
        from services.brain.conversation_store_pg import table_ready
...
def _load_pg(key: str) -> dict[str, Any] | None:
    ...
    with optional_message_session() as session:
        if session is None:
            return None
        from services.brain.conversation_store_pg import pg_get, table_ready
...
def _persist_pg(key: str, tenant_id: str, conversation_id: str, payload: dict[str, Any]) -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.brain.conversation_store_pg import pg_upsert, table_ready
    ...
    if _persist_enabled():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.brain.conversation_store_pg import pg_tenant_ids, table_ready
