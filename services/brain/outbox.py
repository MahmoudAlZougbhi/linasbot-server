...
class OutboxPersistError(Exception):
    """SQL/disk write failed after Brain produced a reply. Do not send or charge."""
...
def reset_outbox_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.brain.outbox_pg import pg_reset, table_ready
...
def _hydrate() -> None:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.brain.outbox_pg import pg_list, pg_outbox_ids, table_ready
...
def _promote_missing_to_pg() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.brain.outbox_pg import pg_get, pg_upsert, table_ready
...
def _get(oid: str) -> OutboxItem | None:
    if not _memory_forced():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.brain.outbox_pg import pg_get, table_ready
    ...
    *,
    ...
    with _LOCK:
        ...
        try:
            ...
            _ITEMS.pop(oid, None)
            try:
                (_root() / f"{oid.replace(':', '_')}.json").unlink(missing_ok=True)
            except Exception:
                pass
            raise OutboxPersistError(f"{type(exc).__name__}: {str(exc)[:200]}") from exc
        return item
...
def known_outbox_tenant_ids() -> list[str]:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.brain.outbox_pg import pg_tenant_ids, table_ready
...
def _list_pg(*, states: tuple[str, ...], tenant_id: str, limit: int) -> list[OutboxItem] | None:
    ...
    with optional_message_session() as session:
        if session is None:
            return None
        from services.brain.outbox_pg import pg_list, table_ready
...
def _sql_known_ids(*, tenant_id: str = "") -> set[str]:
    try:
        ...
        with optional_message_session() as session:
            if session is None:
                return set()
            from services.brain.outbox_pg import pg_outbox_ids, table_ready
...
def outbox_counts(*, tenant_id: str = "") -> dict[str, int]:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.brain.outbox_pg import pg_counts, pg_outbox_ids, table_ready
