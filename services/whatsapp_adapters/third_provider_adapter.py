...
class QiscusAdapter(WhatsAppAdapter):
    ...
    async def send_audio_message(
        ...
        payload = {"to": to_number, "from": self.phone_number_id, "type": "audio", "audio": {"link": audio_url}}
    ...
    async def set_webhook(self, webhook_url: str, events: list | None = None) -> dict[str, Any]:
        ...
        payload = {"url": webhook_url, "events": events or ["message.received", "message.status", "message.failed"]}
    ...
    def _extract_message_content(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if msg_type == "text":
            ...
            return {"audio_id": message.get("audio", {}).get("id") or message.get("media_id")}
