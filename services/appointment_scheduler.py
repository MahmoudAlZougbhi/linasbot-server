...
async def populate_scheduled_messages_from_appointments():
    ...
    try:
        ...
        # ============================================================
        # PHASE 2: Fetch "Done" appointments from yesterday (single API call)
        # Uses /appointments/reminders?status=Done&date=yesterday
        # ============================================================
        yesterday = (today - timedelta(days=1)).date()
        yesterday_str = yesterday.strftime('%Y-%m-%d')  # Format: YYYY-MM-DD for API

        print(f"\n{'='*80}")
        print(f"🔍 PHASE 2: FETCHING 'DONE' APPOINTMENTS FROM YESTERDAY")
        print(f"   API call: /appointments/reminders?status=Done&date={yesterday_str}")
        print(f"{'='*80}")

        done_appointments_found = 0
        done_messages_scheduled = 0

        # Single API call to get all Done appointments from yesterday
        done_result = await send_appointment_reminders(date=yesterday_str, status='Done')

        if done_result.get('success'):
            response_data = done_result.get('data', {})
            if isinstance(response_data, dict):
                done_appointments = response_data.get('appointments', [])
            elif isinstance(response_data, list):
                done_appointments = response_data
            else:
                done_appointments = []

            done_appointments_found = len(done_appointments)
            print(f"   ✅ Found {done_appointments_found} completed appointments from yesterday")

            for apt in done_appointments:
                try:
                    customer_phone = apt.get('phone')
                    customer_name = apt.get('name', 'عميلنا العزيز')
                    apt_details = apt.get('appointment_details', {})
                    ...
                    # Schedule for 6 PM today, or now if past 6 PM
                    six_pm_today = datetime.combine(today.date(), datetime.strptime("18:00", "%H:%M").time())
                    thank_you_time = six_pm_today if today < six_pm_today else today
                    ...
                    if result:
                        done_messages_scheduled += 1
                        print(f"   ✅ {customer_name} ({customer_phone}) - {apt_datetime.strftime('%H:%M')} - scheduled for {thank_you_time.strftime('%H:%M')}")

                except Exception as e:
                    logger.debug(f"Error processing done appointment: {e}")
                    continue
        else:
            print(f"   ❌ Failed to fetch Done appointments: {done_result.get('message', 'Unknown error')}")

        print(f"\n   📊 Phase 2 Results:")
        print(f"   - Done appointments found: {done_appointments_found}")
        print(f"   - Thank-you messages scheduled: {done_messages_scheduled}")
        print(f"{'='*80}")

        total_messages += done_messages_scheduled
