...
from services.qa_manager_service import get_qa_response, qa_manager
...
class EnhancedMessageHandler:
    ...
    def __init__(self) -> None:
        self.user_states: dict[str, dict[str, Any]] = {}  # Track user conversation states
    ...
    async def process_message(
        ...
        get_bot_response_func: Any | None = None,
        is_voice: bool = False,
    ) -> dict:
        """
        ...
        if training_manager.is_admin(user_id):
            ...
            if message.lower() in ["training", "تدريب", "entrainement"]:
                ...
                return {"response": result["message"], "mode": "training_activated", "success": result["success"]}
            ...
            if user_id in training_manager.training_sessions:
                ...
                return {"response": result["message"], "mode": "training", "success": result["success"]}
        ...
        if gender_service.is_waiting_for_gender(user_id):
            ...
            if detected_gender:
                confirmation = gender_service.get_gender_confirmation_message(
                    detected_gender, self._detect_language(message)
                )
                return {"response": confirmation, "mode": "gender_detected", "gender": detected_gender, "success": True}
            else:
        ...
        if not user_gender:
            gender_request = gender_service.create_gender_request(user_id, self._detect_language(message))
            return {"response": gender_request["message"], "mode": "requesting_gender", "success": True}
        ...
        if is_voice:
            ...
            # Adapt for gender
            final_response = gender_service.adapt_response_style(final_response, user_gender, lang)

            return {"response": final_response, "mode": "voice_confirmed", "gender": user_gender, "success": True}
        ...
        if message_type == "text" and not is_voice:
            ...
            async def process_buffered_message(uid: Any, combined_msg: Any) -> None:
                ...
                response = gender_service.adapt_response_style(
                    response, user_gender, self._detect_language(combined_msg)
                )

                # Store for retrieval
                self.user_states[uid] = {"last_response": response, "timestamp": datetime.now()}

            # Add to buffer
            buffered = await message_buffer.add_message(user_id, message, message_type, process_buffered_message)
        ...
        # Adapt for gender
        response = gender_service.adapt_response_style(response, user_gender, self._detect_language(message))

        return {"response": response, "mode": "normal", "gender": user_gender, "success": True}

    def get_buffered_response(self, user_id: str) -> str | None:
