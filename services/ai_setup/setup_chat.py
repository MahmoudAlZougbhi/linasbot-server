...
SECTION_PROMPTS: dict[str, str] = {
    ...
    "requests_appointments": (
        "هل تريد تفعيل طلبات العملاء؟ أضف قواعد طلب: اختر النوع (طلب/موعد/أخرى) مع عنوان وملاحظة. "
        "أو قل تخطي — يبقى المعطّل حتى تنشر قواعد."
    ),
}
...
def section_progress(tenant_id: str) -> list[dict[str, Any]]:
    ...
    from services.ai_setup.progress import list_section_fill_status
...
def apply_section_patch(
    *,
    ...
    from services.billing.membership.edit_http import guarded_cm_write
