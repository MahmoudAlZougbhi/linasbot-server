...
def replace_cm_faq_attachments(
    *,
    ...
    from services.ai_setup.resource_attachment import validate_owner_resource_fields
    from services.ai_setup.schemas import ArticleAttachment
    ...
    *,
    ...
    for item in section.items:
        ...
        if any(v.language == lang and (v.question or "").strip() and (v.answer or "").strip() for v in item.variants):
            ...
            continue
        try:
            from services.billing.membership.processing_budgets import ProcessingBudgetError, consume_attempt
