...
from services.ai_setup.atomic_io import atomic_write_json, compute_checksum, read_json_object
from services.ai_setup.constants import CM_SECTIONS, require_tenant_id
from services.ai_setup.paths import draft_dir, ensure_cm_dirs, tenant_cm_root
from services.ai_setup.schemas import SectionDraftEnvelope, default_section_payload, utc_now
...
def _sanitize_section_payload(section: str, payload: dict[str, object]) -> dict[str, object]:
    ...
    if section == "languages":
        from services.ai_setup.language_policy import sanitize_languages_payload

        return sanitize_languages_payload(payload)  # type: ignore[arg-type]
    if section == "dynamic_messages":
        from services.ai_setup.greeting_rules import sanitize_dynamic_messages_payload
