"""Luna (gpt-5.6-luna) chunks large owner text and corrects typos without new facts."""
...
logger = logging.getLogger("customer_ai.luna_chunker")
...
def luna_chunk_prose(request: dict[str, Any]) -> list[dict[str, str]]:
    ...
    try:
        ...
        logger.exception("luna_chunk_failed section=%s", request.get("section"))
    try:
        chunks = _luna_chunks({**request, "retry": True})
        if chunks:
            return _normalize_chunks(chunks)
    except Exception:
        logger.exception("luna_chunk_retry_failed section=%s", request.get("section"))
    raise MetadataPreparationError()


def _llm_enabled() -> bool:
    from services.search_metadata.generate import _llm_enabled as metadata_llm_enabled
...
def _luna_chunks(request: dict[str, Any]) -> list[dict[str, str]]:
    ...
    system = (
        "You prepare INTERNAL retrieval chunks for Linas AI (Luna). "
