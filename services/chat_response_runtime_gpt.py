...
async def call_first_gpt(ns: Any) -> Any:
    ...
    tools = list(get_openai_tools_schema(excluded_tool_names=excluded))
    try:
        from services.chat_response_runtime_common import config as _cfg
        from services.requests.ai_tool import tools_for_tenant

        ud = _cfg.user_data_whatsapp.get(ns.user_id) or {}
        tenant_id = str(ud.get("tenant_id") or ud.get("tenantId") or "").strip()
        for tool in tools_for_tenant(tenant_id):
            name = str((tool.get("function") or {}).get("name") or "")
            if name and all(str((t.get("function") or {}).get("name") or "") != name for t in tools):
                tools.append(tool)
    except Exception as exc:
        print(f"[chat_response] requests capture tool registration skipped: {exc}")
