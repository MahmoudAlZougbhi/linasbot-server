...
def build_patch_preview(*, tenant_id: str, section: str, patch: dict[str, Any]) -> dict[str, Any]:
    ...
    from services.ai_setup.setup_chat import _merge_dict
...
def approve_cm_patch(
    *,
    ...
    if delete_ids is not None and str((prop.preview or {}).get("kind") or "") == "cm_delete":
        from services.owner_copilot.tools_cm_delete import filter_delete_targets
...
async def approve_cm_patch_and_activate(
    *,
    ...
    base["live"] = bool(activation.get("activated"))
    return base
