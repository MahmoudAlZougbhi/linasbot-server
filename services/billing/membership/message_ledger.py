...
from services.billing.membership.lot_window import lot_is_live
from services.billing.membership.message_policy import ResponseClass, message_units_for
    ...
    *,
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_grant_lot
...
def snapshot(tenant_id: str) -> LedgerSnapshot:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_snapshot
...
def expire_included_before(tenant_id: str, period_id: str) -> int:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_expire_included_before
...
def settle(*, tenant_id: str, operation_id: str, accepted: bool, units: int | None = None) -> MessageReservation:
    ...
    with _pg_session() as session:
        if session is not None:
            ...
            try:
                return pg_settle(session, tenant_id=tid, operation_id=operation_id, accepted=accepted, units=units)
            except KeyError:
                pass
    with _LOCK:
        ...
        if charge:
            ...
            if lot is None or _live_remaining(lot) < charge:
                lot = _pick_lot(_LOTS.get(tid, []))
            if lot is None or _live_remaining(lot) < charge:
                raise InsufficientMessages(tid, lot.remaining if lot else 0)
            lot.remaining -= charge
...
def reverse(*, tenant_id: str, operation_id: str) -> MessageReservation:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_reverse
...
def revoke_purchased(*, tenant_id: str, transaction_id: str) -> dict[str, Any]:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_revoke_purchased
...
def list_stale_reserved(
    *,
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_list_stale_reserved
...
def list_reservations(tenant_id: str | None = None) -> list[MessageReservation]:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_list_reservations
...
def known_ledger_tenant_ids() -> list[str]:
    ...
    with _pg_session() as session:
        if session is not None:
            from services.billing.membership.message_ledger_pg import pg_known_tenant_ids
