...
from services.billing.membership.message_catalog import AI_SETUP_DAILY_EDIT_DEFAULT
...
def _ready_session(kind: str) -> Iterator[Any | None]:
    from services.billing.membership.pg_store import memory_forced, optional_message_session
    ...
    with optional_message_session() as session:
        if session is None:
            ...
            return
        if kind == "policy":
            from services.billing.membership.daily_edits_policy_pg import table_ready
        else:
            from services.billing.membership.daily_edits_pg import table_ready
...
def set_platform_baseline(limit: int) -> None:
    ...
    with _ready_session("policy") as session:
        if session is not None:
            from services.billing.membership.daily_edits_policy_pg import PLATFORM_TENANT, pg_upsert_limit
...
def set_tenant_override(tenant_id: str, limit: int | None) -> None:
    ...
    with _ready_session("policy") as session:
        if session is None:
            return
        from services.billing.membership.daily_edits_policy_pg import pg_delete_limit, pg_upsert_limit
...
def effective_limit(tenant_id: str) -> tuple[int, str]:
    ...
    with _ready_session("policy") as session:
        if session is not None:
            from services.billing.membership.daily_edits_policy_pg import pg_load_limits
...
def status(tenant_id: str, *, now: datetime | None = None) -> DailyEditDecision:
    ...
    with _ready_session("edits") as session:
        if session is not None:
            from services.billing.membership.daily_edits_pg import pg_counts, pg_operation_ids
...
def known_daily_edit_tenant_ids() -> list[str]:
    ...
    with _ready_session("edits") as session:
        if session is not None:
            from services.billing.membership.daily_edits_pg import pg_tenant_ids

            ids.update(pg_tenant_ids(session, window_id=window_id))
    with _ready_session("policy") as session:
        if session is not None:
            from services.billing.membership.daily_edits_policy_pg import PLATFORM_TENANT, pg_load_limits
...
def reserve_edit(
    *,
    ...
    with _ready_session("edits") as session:
        if session is not None:
            from services.billing.membership.daily_edits_pg import pg_counts, pg_operation_ids, pg_reserve
...
def commit_edit(*, tenant_id: str, operation_id: str, now: datetime | None = None) -> DailyEditDecision:
    ...
    with _ready_session("edits") as session:
        if session is not None:
            from services.billing.membership.daily_edits_pg import pg_commit, pg_operation_ids
...
def release_edit(*, tenant_id: str, operation_id: str, now: datetime | None = None) -> DailyEditDecision:
    ...
    with _ready_session("edits") as session:
        if session is not None:
            from services.billing.membership.daily_edits_pg import pg_operation_ids, pg_release
