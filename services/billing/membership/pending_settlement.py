...
def reset_pending_settlements_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.pending_settlement_pg import pg_reset, table_ready
...
def _memory_forced() -> bool:
    from services.billing.membership.pg_store import memory_forced
...
def _hydrate() -> None:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.billing.membership.pending_settlement_pg import pg_list, pg_settlement_ids, table_ready
...
def _sql_ready() -> bool:
    ...
    with optional_message_session() as session:
        if session is None:
            return False
        from services.billing.membership.pending_settlement_pg import table_ready
...
def _get(sid: str) -> PendingSettlement | None:
    if not _memory_forced():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.billing.membership.pending_settlement_pg import pg_get, table_ready
...
def _persist_pg(item: PendingSettlement) -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.pending_settlement_pg import pg_upsert, table_ready
...
def _list_pg(
    *,
    ...
    with optional_message_session() as session:
        if session is None:
            return None
        from services.billing.membership.pending_settlement_pg import pg_list, table_ready
...
def _sql_known_ids(*, tenant_id: str = "") -> set[str]:
    try:
        ...
        with optional_message_session() as session:
            if session is None:
                return set()
            from services.billing.membership.pending_settlement_pg import pg_settlement_ids, table_ready
...
def get_pending(tenant_id: str, reservation_id: str, operation_id: str = "") -> PendingSettlement | None:
    ...
    with optional_message_session() as session:
        if session is None:
            return None
        from services.billing.membership.pending_settlement_pg import (
            pg_get_by_alias,
            pg_get_by_reservation,
            table_ready,
        )
...
def policy_for_operation(tenant_id: str, *operation_ids: str) -> BillingPolicy | None:
    ...
    if not _memory_forced():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.billing.membership.pending_settlement_pg import pg_policy_for, table_ready
...
def known_settlement_tenant_ids() -> list[str]:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.billing.membership.pending_settlement_pg import pg_tenant_ids, table_ready
...
def pending_counts(*, tenant_id: str = "") -> dict[str, int]:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.billing.membership.pending_settlement_pg import pg_counts, pg_settlement_ids, table_ready
