...
def reset_credit_reservation_index_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.credit_reservation_index_pg import pg_reset, table_ready
...
def _hydrate() -> None:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.billing.membership.credit_reservation_index_pg import (
                pg_list,
                pg_reservation_ids,
                table_ready,
            )
...
def _promote_missing_to_pg() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.credit_reservation_index_pg import pg_get, pg_upsert, table_ready
...
def _persist_pg(item: OpenCreditReservation) -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.credit_reservation_index_pg import pg_upsert, table_ready
...
def _get(reservation_id: str) -> OpenCreditReservation | None:
    if not _memory_forced():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.billing.membership.credit_reservation_index_pg import pg_get, table_ready
...
def _list_pg(*, tenant_id: str = "", state: str = "reserved") -> list[OpenCreditReservation] | None:
    ...
    with optional_message_session() as session:
        if session is None:
            return None
        from services.billing.membership.credit_reservation_index_pg import pg_list, table_ready
...
def _sql_known_ids(*, tenant_id: str = "") -> set[str]:
    ...
    with optional_message_session() as session:
        if session is None:
            return set()
        from services.billing.membership.credit_reservation_index_pg import pg_reservation_ids, table_ready
...
def known_index_tenant_ids() -> list[str]:
    ...
    if not _memory_forced():
        ...
        with optional_message_session() as session:
            if session is not None:
                from services.billing.membership.credit_reservation_index_pg import pg_tenant_ids, table_ready
