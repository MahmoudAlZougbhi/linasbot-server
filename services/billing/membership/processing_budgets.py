...
def reset_processing_budgets_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.processing_budgets_pg import pg_reset, table_ready
...
def begin_job(tenant_id: str, *, limit: int = DEFAULT_CONCURRENT_JOBS) -> BudgetDecision:
    ...
    with _session() as session:
        if session is not None:
            from services.billing.membership.processing_budgets_pg import pg_begin_job, pg_job_count, table_ready
...
def end_job(tenant_id: str) -> None:
    ...
    with _session() as session:
        if session is None:
            return
        from services.billing.membership.processing_budgets_pg import pg_end_job, table_ready
...
def consume_attempt(tenant_id: str, *, limit: int = DEFAULT_DAILY_PROVIDER_ATTEMPTS) -> BudgetDecision:
    ...
    with _session() as session:
        if session is not None:
            from services.billing.membership.processing_budgets_pg import (
                pg_attempt_used,
                pg_consume_attempt,
                table_ready,
            )
...
def known_processing_tenant_ids() -> list[str]:
    ...
    with _session() as session:
        if session is not None:
            from services.billing.membership.processing_budgets_pg import pg_tenant_ids, table_ready
