...
def reset_admin_state_for_tests() -> None:
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.catalog_admin_pg import pg_reset, table_ready
...
def load_admin_state() -> dict[str, Any] | None:
    ...
    with optional_message_session() as session:
        if session is not None:
            from services.billing.membership.catalog_admin_pg import pg_load, pg_upsert, table_ready
...
def save_admin_state(
    *,
    ...
    with optional_message_session() as session:
        if session is None:
            return
        from services.billing.membership.catalog_admin_pg import pg_upsert, table_ready
