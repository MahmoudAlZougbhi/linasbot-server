...
def reset_catalog_admin_for_tests() -> None:
    with _LOCK:
        ...
        _AUDIT.clear()
        _LOADED = True
    from services.billing.membership.catalog_admin_store import reset_admin_state_for_tests
...
def _hydrate_unlocked() -> None:
    if _LOADED:
        return
    from services.billing.membership.catalog_admin_store import load_admin_state

    _apply_state(load_admin_state())


def _refresh_unlocked() -> None:
    from services.billing.membership.catalog_admin_store import load_admin_state, persist_enabled
...
def _persist_unlocked() -> None:
    from services.billing.membership.catalog_admin_store import save_admin_state
