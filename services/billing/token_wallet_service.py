...
from services.billing.token_wallet_models import (  # noqa: F401
...
class TokenWalletService:
    ...
    def _read(self, tenant_id: str) -> dict[str, Any]:
        if billing_uses_postgres():
            from services.billing.token_wallet_pg_store import read_wallet
    ...
    def _persist_wallet(self, tenant_id: str, data: dict[str, Any], *, migration_ledger: bool = False) -> None:
        if billing_uses_postgres():
            from services.billing.token_wallet_pg_store import append_ledger, write_wallet
    ...
    def _append_ledger(self, entry: dict[str, Any]) -> None:
        if billing_uses_postgres():
            from services.billing.token_wallet_pg_store import append_ledger
    ...
    def credit(
        ...
        *,
        ...
        if billing_uses_postgres():
            from services.billing.token_wallet_pg_store import append_ledger, read_wallet, write_wallet
    ...
    def recent_ledger(self, tenant_id: str, *, limit: int = 50) -> list[dict[str, Any]]:
        tid = normalize_wallet_tenant_id(tenant_id)
        if billing_uses_postgres():
            from services.billing.token_wallet_pg_store import recent_ledger
