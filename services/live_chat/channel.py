...
def live_chat_event_tenant_id(user_id: Any, payload: dict[str, Any] | None = None) -> str:
    """Proven tenant for SSE fanout. Empty when the id cannot prove a workspace."""
    from services.live_chat.tenant import resolve_live_chat_tenant_id

    return resolve_live_chat_tenant_id(user_id=user_id, payload=payload)
