...
# User rate tracking
class _RateTracker(TypedDict):
    messages: list[datetime]
    images: list[datetime]
    voice: list[datetime]
    last_reset: datetime


user_rate_tracker: defaultdict[str, _RateTracker] = defaultdict(
    lambda: {"messages": [], "images": [], "voice": [], "last_reset": datetime.now()}
)
...
async def moderate_content(text: str, user_id: str | None = None) -> tuple[bool, dict]:
    """
    ...
    try:
        ...
        if result.flagged:
            ...
            if is_self_harm_flag and self_harm_score < 0.5:
                print(
                    f"✅ MODERATION: Low confidence self-harm flag ({self_harm_score:.2f}) for user {user_id} - allowing"
                )
                return True, {
                    "flagged": False,
                    "low_confidence": True,
        ...
        if "429" in error_str or "Too Many Requests" in error_str:
            print("⚠️ OpenAI moderation API rate limit hit - allowing content through")
            # Allow content through when moderation API is rate limited
            return True, {"flagged": False, "error": str(e), "message": "Moderation API rate limited - content allowed"}

        # For other errors, be conservative and flag as potentially unsafe
        return False, {"flagged": True, "error": str(e), "message": "Could not verify content safety"}
...
async def check_rate_limits(user_id: str, message_type: str = "message") -> tuple[bool, str]:
    ...
    tracker["messages"] = [t for t in tracker["messages"] if t > cutoff_time]
    tracker["images"] = [t for t in tracker["images"] if t > cutoff_time]
    tracker["voice"] = [t for t in tracker["voice"] if t > cutoff_time]
    ...
    if message_type == "message":
        ...
        if messages_last_minute > RATE_LIMITS["messages_per_minute"]:
            return (
                False,
                f"Rate limit exceeded: Too many messages per minute (limit: {RATE_LIMITS['messages_per_minute']})",
            )
        ...
        if voice_last_hour > RATE_LIMITS["voice_per_hour"]:
            return (
                False,
                f"Rate limit exceeded: Too many voice messages per hour (limit: {RATE_LIMITS['voice_per_hour']})",
            )

    return True, ""


def log_violation(user_id: str, content: str, categories: dict) -> None:
    """
...
async def moderate_image_prompt(prompt: str, user_id: str | None = None) -> tuple[bool, dict]:
    """
...
async def get_moderation_stats() -> dict:
    ...
    try:
        ...
        if not os.path.exists(log_file):
            return {"total_violations": 0, "violations_today": 0, "top_categories": []}
        ...
        return {"total_violations": 0, "violations_today": 0, "top_categories": [], "error": str(e)}
