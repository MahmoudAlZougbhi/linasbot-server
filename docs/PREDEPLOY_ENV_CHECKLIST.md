...
BOC booking (`BOC_BOOKING_ENABLED`) defaults **off**. When off, readiness must not require BOC token or booking IDs (`checks.boc_booking`). See `docs/requests/BOC_FUTURE_INTEGRATION.md`. Do not enable BOC in production without owner approval.
