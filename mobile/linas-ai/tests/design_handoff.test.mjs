...
test('chat AI and user bubbles keep size, medium fill like ChatGPT', () => {
  const typography = read('theme/typography.ts');
  const bubble = read('features/chat/ChatBubble.tsx');
  const aiBody = read('features/chat/AiMessageBody.tsx');
  assert.match(typography, /chatAi:[\s\S]*?fontFamily:\s*bodyMedium/);
  assert.match(typography, /chatAi:[\s\S]*?fontSize:\s*17/);
  assert.match(typography, /chatAi:[\s\S]*?lineHeight:\s*24/);
  assert.doesNotMatch(typography, /chatAi:[\s\S]*?fontSize:\s*1[89]/);
  assert.match(typography, /chatUser:[\s\S]*?fontFamily:\s*bodyMedium/);
...
test('Dashboard sections match design handoff', () => {
  ...
  assert.doesNotMatch(table, /fontWeight:\s*'700'/);
  ...
  assert.match(header, /fonts\.body/);
  assert.match(grid, /fontSize: 16/);
  assert.match(table, /fontFamily: fonts\.bodyMedium, fontSize: 16/);
  assert.match(copilot, /fontFamily: fonts\.bodyMedium, fontSize: 16/);
  const shared = read('features/shared/ScreenChrome.tsx');
  assert.doesNotMatch(shared, /titleBlock: \{[\s\S]*?paddingTop:\s*10/);
