...
export type KnowledgeResourcePrompt = {
  mode: 'create' | 'edit';
  kind: KnowledgeKind;
  attachId?: string;
  replaceId?: string;
...
export function useKnowledgeMedia(
  selected: KnowledgeItem | null,
  patchSelected: (patch: Partial<Pick<KnowledgeItem, 'attachments'>>) => void,
  tr: (key: StringKey) => string,
) {
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [prompt, setPrompt] = useState<KnowledgeResourcePrompt | null>(null);
