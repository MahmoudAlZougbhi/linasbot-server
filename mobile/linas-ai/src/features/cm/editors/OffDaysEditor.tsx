...
export function OffDaysEditor({ payload, onChange }: EditorProps) {
  ...
  const toggleDate = (ymd: string) => {
    ...
    if (existing) {
      onChange({
        ...payload,
        rules: rules.filter((r) => String(r.id) !== String(existing.id)),
      });
      return;
    }
    onChange({
  ...
  return (
    <View>
      <Text style={cmFormStyles.hint}>
        Tap a day to mark it off (red). Save so the AI treats those days as closed. Update anytime.
      </Text>
      <View style={cmFormStyles.card}>
        <View style={cmFormStyles.row}>
          <Pressable onPress={() => setMonthCursor(addMonths(monthCursor, -1))}>
            <Text style={cmFormStyles.chipText}>‹ Prev</Text>
          </Pressable>
          <Text style={cmFormStyles.itemTitle}>{title}</Text>
          <Pressable onPress={() => setMonthCursor(addMonths(monthCursor, 1))}>
            <Text style={cmFormStyles.chipText}>Next ›</Text>
          </Pressable>
        </View>
        ...
        <Text style={[cmFormStyles.hint, { marginTop: 12 }]}>
          {offDates.size} closed day{offDates.size === 1 ? '' : 's'} marked
        </Text>
      </View>
    </View>
  );
