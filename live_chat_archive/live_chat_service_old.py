"""Live Chat service (rewritten) with canonical state, index sync, and realtime helpers."""
...
class LiveChatService:
    """Core live chat backend (Firestore source of truth, live_chat_index projection)."""
    ...
    ACTIVE_TIME_WINDOW = 6 * 60 * 60
    FIRESTORE_FETCH_PARALLELISM = 16
    ...
    def _users_collection(self, db):
        return db.collection("artifacts").document(self.APP_ID).collection("users")

    def _normalize_conversation_state(self, data: Dict[str, Any]) -> str:
        state = str(data.get("conversation_state") or "").strip().lower()
        if state in {
            self.STATE_ASSIGNED,
    ...
    def _resolve_user_phone(self, user_id: str, customer_info: Dict[str, Any]) -> Tuple[str, str]:
        phone_full = customer_info.get("phone") or user_id
        phone_clean = normalize_phone(phone_full) or phone_full
        return phone_full, phone_clean

    def _build_index_entry(self, user_id: str, conv_data: dict) -> dict:
        ...
        if isinstance(last_ts, str):
            try:
                last_ts = parse_timestamp_utc(last_ts)
            except Exception:
                last_ts = utc_now()

        visible_messages = conv_data.get("messages", [])
        last_message = visible_messages[-1] if visible_messages else {}
        last_message_text = last_message.get("text", "") if isinstance(last_message, dict) else ""
        ...
        return {
            ...
            "is_live": is_live,
            "last_updated": conv_data.get("last_updated") or last_ts,
        }
    ...
    async def _upsert_index_entry(self, entry: dict):
        ...
        if not db:
            return
        idx_coll = self._index_collection(db)
        doc_ref = idx_coll.document(entry.get("conversation_id"))
        await asyncio.to_thread(doc_ref.set, entry)
    ...
    async def _sync_index_from_source(self, user_id: str, conversation_id: str, *, allow_state_backfill: bool = False) -> Dict[str, Any]:
        ...
        conv_data = normalize_conversation_document(conversation_id, canonical_user_id, raw_payload)
        conv_data["conversation_state"] = self._normalize_conversation_state(conv_data)
        ...
        entry = self._build_index_entry(canonical_user_id, conv_data)
        await self._upsert_index_entry(entry)

        return {"written": True, "state_backfill": state_backfill}
    ...
    async def rebuild_index_from_firestore(
        ...
        users_collection = self._users_collection(db)
        if users_collection is None:
            print("⚠️ Users collection missing; cannot rebuild index")
            return 0

        users_docs = await asyncio.to_thread(lambda: list(users_collection.stream() if max_users is None else users_collection.limit(max_users).stream()))
        user_ids = [doc.id for doc in users_docs]

        semaphore = asyncio.Semaphore(self.FIRESTORE_FETCH_PARALLELISM)

        async def _fetch_user_conversations(uid: str):
            async with semaphore:
                try:
                    conv_coll = users_collection.document(uid).collection(config.FIRESTORE_CONVERSATIONS_COLLECTION)
                    q = conv_coll.order_by("last_updated", direction=firestore.Query.DESCENDING)
                    if max_conversations_per_user:
                        q = q.limit(max_conversations_per_user)
                    docs = await asyncio.to_thread(lambda: list(q.stream()))
                    return uid, docs
                except Exception as exc:
                    print(f"⚠️ Error streaming conversations for {uid}: {exc}")
                    return uid, []

        conversation_results = await asyncio.gather(*[_fetch_user_conversations(uid) for uid in user_ids])

        total_written = 0
        repaired_states = 0
        skipped = 0

        for uid, conv_docs in conversation_results:
        ...
        print(
            f"📦 Rebuilt live_chat_index entries: written={total_written} repaired_states={repaired_states} skipped={skipped}"
        )

        if return_details:
            return {"written": total_written, "repaired_states": repaired_states, "skipped": skipped}
        return total_written

    # Inbox / queues ---------------------------------------------------

    async def _load_index_entries(self) -> List[Dict[str, Any]]:
        db = get_firestore_db()
        if not db:
            return []
        idx_coll = self._index_collection(db)
        docs = await asyncio.to_thread(lambda: list(idx_coll.order_by("last_message_at", direction=firestore.Query.DESCENDING).stream()))
        entries = []
        for doc in docs:
            data = doc.to_dict() or {}
    ...
    def _filter_state_values(self, filter_key: str) -> List[str]:
    ...
    def _match_search(self, entry: Dict[str, Any], needle: str) -> bool:
        if not needle:
            return True
        n = needle.lower()
        return any(
            n in str(entry.get(key, "")).lower()
            for key in ["customer_name", "phone", "phone_clean", "conversation_id", "last_message_text"]
        )
    ...
    async def get_unified_chats(
        ...
        *,
        ...
        allowed_states = self._filter_state_values(filter_state)
        filtered = []
        for e in entries:
            if allowed_states and e.get("conversation_state") not in allowed_states:
                continue
            if not self._match_search(e, search):
                continue
            filtered.append(e)
    ...
    async def get_active_conversations(self, search: str = "") -> List[Dict[str, Any]]:
        ...
        return [e for e in entries if e.get("conversation_state") in active_states and self._match_search(e, search)]

    async def get_waiting_queue(self) -> List[Dict[str, Any]]:
        entries = await self._load_index_entries()
        return [e for e in entries if e.get("conversation_state") == self.STATE_WAITING_OPERATOR]

    # Actions -----------------------------------------------------------

    async def takeover_conversation(self, conversation_id: str, user_id: str, operator_id: str) -> Dict[str, Any]:
        db = get_firestore_db()
        if not db:
            return {"success": False, "error": "firestore_missing"}

        canonical_user, _ = get_canonical_user_id_and_phone(user_id)
        conv_ref = (
            db.collection("artifacts")
            .document(self.APP_ID)
            .collection("users")
            .document(canonical_user)
            .collection(config.FIRESTORE_CONVERSATIONS_COLLECTION)
            .document(conversation_id)
        )

        await asyncio.to_thread(conv_ref.update, {"operator_id": operator_id, "human_takeover_active": True, "conversation_state": self.STATE_ASSIGNED})
        await self._sync_index_from_source(canonical_user, conversation_id, allow_state_backfill=True)
        return {"success": True}
    ...
    async def release_conversation(self, conversation_id: str, user_id: str) -> Dict[str, Any]:
        ...
        await asyncio.to_thread(conv_ref.update, {"operator_id": None, "human_takeover_active": False, "conversation_state": self.STATE_BOT_ACTIVE})
        await self._sync_index_from_source(canonical_user, conversation_id, allow_state_backfill=True)
        return {"success": True}
    ...
    async def send_operator_message(
        ...
        await asyncio.to_thread(conv_ref.update, update_payload)
    ...
    async def update_message_content(
        ...
        updated = False
        for msg in messages:
            if str(msg.get("message_id")) == str(message_id) or str(msg.get("metadata", {}).get("source_message_id")) == str(message_id):
        ...
        await asyncio.to_thread(conv_ref.update, {"messages": messages, "last_updated": utc_now()})
        await self._sync_index_from_source(canonical_user, conversation_id, allow_state_backfill=True)
        return {"success": True}
    ...
    async def get_conversation_details(
        ...
        *,
        ...
        data = normalize_conversation_document(conversation_id, canonical_user, snap.to_dict() or {})
        messages = data.get("messages", []) or []

        cutoff = utc_now() - datetime.timedelta(days=days or 1)
        if before:
            cutoff = parse_timestamp_utc(before) - datetime.timedelta(days=day_window or 1)
        filtered_msgs = [m for m in messages if parse_timestamp_utc(m.get("timestamp")) >= cutoff]
        filtered_msgs = filtered_msgs[-max_messages:]
    ...
    async def get_client_conversations(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        docs = await asyncio.to_thread(lambda: list(conv_coll.order_by("last_updated", direction=firestore.Query.DESCENDING).stream()))
        conversations = []
        for doc in docs:
            data = normalize_conversation_document(doc.id, canonical_user, doc.to_dict() or {})
            data["conversation_state"] = self._normalize_conversation_state(data)
            conversations.append(data)
        return conversations

    async def get_faq_match_context(self, user_id: str, conversation_id: str, message_id: str) -> Dict[str, Any]:
        # Placeholder: return minimal structure to satisfy UI expectations.
        return {"success": True, "match": None, "conversation_id": conversation_id, "message_id": message_id}

    async def get_metrics(self) -> Dict[str, Any]:
        entries = await self._load_index_entries()
        counts = defaultdict(int)
        for e in entries:
            counts[e.get("conversation_state") or "unknown"] += 1
