...
async def text_handlers_respond_phase2(ctx: dict):
    ...
    _pack = ['DEFAULT_TENANT_ID', 'UNPUBLISHED_AI_MESSAGE', '_build_out_of_scope_reply', '_ge', '_ginf', '_handle_published_cm_runtime', '_is_out_of_clinic_scope_query', 'ai_primary_mode', 'cm_diag', 'cm_metadata', 'cm_reply', 'cm_steps', 'cm_tenant_id', 'conv_state', 'current_conversation_id', 'current_gender', 'current_preferred_lang', 'get_gender_from_message', 'k', 'lang_key', 'log_interaction', 'out_of_scope_reply', 'response_language', 'router_action', 'router_route', 'save_conversation_message_to_firestore', 'send_message_func', 'tenant_allows_legacy_bridge', 'tenant_uses_cm_runtime', 'unpublished_reply', 'user_data', 'user_id', 'user_image_base64', 'user_input_to_process', 'user_name', 'user_persistence', 'v']
    for _k in _pack:
        if _k in locals():
            ctx[_k] = locals()[_k]
    return None
