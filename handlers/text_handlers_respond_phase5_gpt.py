...
async def text_handlers_respond_phase5_gpt(pctx: dict):
    _apply_turn_by_turn_policy = pctx.get("_apply_turn_by_turn_policy")
    _build_arabic_respectful_address = pctx.get("_build_arabic_respectful_address")
    _is_price_intent = pctx.get("_is_price_intent")
    _resume_original_question = pctx.get("_resume_original_question")
    ai_primary_mode = pctx.get("ai_primary_mode")
    canonical_user_id = pctx.get("canonical_user_id")
    conv_state = pctx.get("conv_state")
    conversation_history = pctx.get("conversation_history")
    current_conversation_id = pctx.get("current_conversation_id")
    current_gender = pctx.get("current_gender")
    current_preferred_lang = pctx.get("current_preferred_lang")
    detect_reschedule_intent = pctx.get("detect_reschedule_intent")
    firestore_conversation_id = pctx.get("firestore_conversation_id")
    get_bot_chat_response = pctx.get("get_bot_chat_response")
    get_canonical_user_id_and_phone = pctx.get("get_canonical_user_id_and_phone")
    get_conversation_context_for_gpt = pctx.get("get_conversation_context_for_gpt")
    get_conversation_last_ai_response_at = pctx.get("get_conversation_last_ai_response_at")
    get_dynamic_message = pctx.get("get_dynamic_message")
    get_last_bot_message_for_gpt_context = pctx.get("get_last_bot_message_for_gpt_context")
    gpt_response_data = pctx.get("gpt_response_data")
    initial_user_query_to_process_original = pctx.get("initial_user_query_to_process_original")
    is_initial_message_for_gpt = pctx.get("is_initial_message_for_gpt")
    is_post_takeover_escalation_cooldown = pctx.get("is_post_takeover_escalation_cooldown")
    last_ai_response_at = pctx.get("last_ai_response_at")
    local_qa_service = pctx.get("local_qa_service")
    log_interaction = pctx.get("log_interaction")
    query_to_send_to_gpt = pctx.get("query_to_send_to_gpt")
    respectful_address = pctx.get("respectful_address")
    response_language = pctx.get("response_language")
    save_conversation_message_to_firestore = pctx.get("save_conversation_message_to_firestore")
    save_for_training_conversation_log = pctx.get("save_for_training_conversation_log")
    send_message_func = pctx.get("send_message_func")
    update_dashboard_metric_in_firestore = pctx.get("update_dashboard_metric_in_firestore")
    user_data = pctx.get("user_data")
    user_id = pctx.get("user_id")
    user_image_base64 = pctx.get("user_image_base64")
    user_image_format = pctx.get("user_image_format")
    user_input_to_process = pctx.get("user_input_to_process")
    user_name = pctx.get("user_name")
    is_reschedule_intent = pctx.get("is_reschedule_intent")
    is_price_intent = pctx.get("is_price_intent")
    if ai_primary_mode:
