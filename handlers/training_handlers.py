...
from utils.utils import notify_human_on_whatsapp, save_for_training_conversation_log
...
async def start_training_mode(user_id: str, user_data: dict, send_message_func: Any, send_action_func: Any) -> None:
    """
...
async def exit_training_mode(user_id: str, user_data: dict, send_message_func: Any, send_action_func: Any) -> None:
    """
...
async def save_generated_data_to_log(user_id: str, send_message_func: Any) -> None:
    """
    ...
    if not data_to_save:
        await send_message_func(
            user_id,
            "لا توجد بيانات تدريب مولدة حالياً لحفظها. يرجى توليدها أولاً (عبر طلب 'سؤال وجواب عن كذا' أو 'لخص').",
        )
        return
    ...
    try:
        ...
        for entry in data_to_save:
            if isinstance(entry, dict) and "question" in entry and "answer" in entry and "language" in entry:
                ...
                await send_message_func(
                    user_id, f"تحذير: عنصر غير صالح في بيانات التدريب المولدة، لن يتم حفظه: {entry}"
                )
        ...
        notify_human_on_whatsapp(
            config.user_names.get(user_id, "غير محدد"),
...
SendMessageFunc = Callable[[str, str], Awaitable[None]]
SendActionFunc = Callable[[str], Awaitable[None]]


async def handle_training_input(
    user_id: str,
    user_name: str = "مدرب",
    user_input_text: str | None = None,
    audio_data_bytes: io.BytesIO | None = None,
    image_url: str | None = None,
    user_data: dict | None = None,
    send_message_func: SendMessageFunc | None = None,
    send_action_func: SendActionFunc | None = None,
) -> None:
    """
    Handles input from the trainer in training mode, supporting text, voice, and photo.
    """
    if send_message_func is None or send_action_func is None or user_data is None:
        return
    ...
    if audio_data_bytes:
        if not PYDUB_AVAILABLE:
            await send_message_func(
                user_id, "عذراً، معالجة الرسائل الصوتية غير متاحة حالياً. الرجاء إرسال تعليماتك نصياً."
            )
            return
        ...
        try:
            ...
            await send_message_func(
                user_id,
                "🚫 آسفة، لم أستطع فهم رسالتك الصوتية لتعليمات التدريب. الرجاء المحاولة مرة أخرى أو الكتابة نصياً.",
            )
            return
        ...
        if (
            ...
            await send_message_func(
                user_id,
                "وصلت لعدد الصور القصوى المسموحة في وضع التدريب أيضاً. يرجى الخروج من وضع التدريب أو التواصل مع المطور.",
            )
            return
        try:
            ...
            bot_initial_reply, analysis_data = await get_bot_photo_analysis_from_gpt(
                user_id, base64_image, is_training_quiz=True
            )
    ...
    if gpt_response_obj and "data" in gpt_response_obj:
        if isinstance(gpt_response_obj["data"], list):  # If JSON Q&A
            user_data["training_conversation_context"].append(
                {"role": "assistant", "content": json.dumps(gpt_response_obj["data"], ensure_ascii=False)}
            )
        elif isinstance(gpt_response_obj["data"], str):  # If plain text
            user_data["training_conversation_context"].append(
                {"role": "assistant", "content": gpt_response_obj["data"]}
            )
        else:
            user_data["training_conversation_context"].append(
                {"role": "assistant", "content": "Unrecognized GPT response format."}
            )
    ...
    if gpt_response_obj and gpt_response_obj["type"] == "qa_list":
        ...
        )  # parse_mode='Markdown' might not be supported by WhatsApp text
