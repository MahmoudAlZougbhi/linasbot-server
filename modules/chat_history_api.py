# -*- coding: utf-8 -*-
"""
Chat History API module: Chat history endpoints
Handles retrieval of customer conversations and chat history.
"""
...
_customers_cache_time = None
CACHE_TTL = 120  # seconds


@app.get("/api/chat-history/customers")
async def get_chat_history_customers():
    ...
    try:
        ...
        if (_customers_cache is not None and
            _customers_cache_time is not None and
            (current_time - _customers_cache_time).total_seconds() < CACHE_TTL):
        ...
        async def fetch_user_conversations(user_doc):
            ...
            try:
                ...
                return (user_doc, conversations_docs)
            except Exception as e:
                print(f"⚠️ Error fetching conversations for user {user_doc.id}: {e}")
                return (user_doc, [])
        ...
        for result in conversation_results:
            ...
            user_doc, conversations_docs = result
            user_id = user_doc.id
            user_data = user_doc.to_dict()
            ...
            if conversations_docs:
                ...
                if latest_conversation:
                    ...
                    if customer_name == "Unknown Customer":
                        # Check local config first, then fall back to phone number
                        customer_name = config.user_names.get(user_id, phone_full or user_id)
        ...
        _customers_cache_time = current_time
...
@app.get("/api/chat-history/conversations/{user_id}")
async def get_customer_conversations(user_id: str):
    ...
    try:
        ...
        for doc in docs:
            ...
            # Get last message for preview (with formatted timestamp)
            last_message = None
            if messages:
                ...
                if isinstance(timestamp, str):
                    try:
                        last_msg["timestamp"] = datetime.datetime.fromisoformat(timestamp.replace('Z', '+00:00')).isoformat()
                    except:
                        last_msg["timestamp"] = datetime.datetime.now().isoformat()
                elif hasattr(timestamp, 'timestamp'):
                    last_msg["timestamp"] = datetime.datetime.fromtimestamp(timestamp.timestamp()).isoformat()
                elif hasattr(timestamp, 'seconds'):
                    last_msg["timestamp"] = datetime.datetime.fromtimestamp(timestamp.seconds).isoformat()
                else:
                    last_msg["timestamp"] = datetime.datetime.now().isoformat()
...
@app.get("/api/chat-history/messages/{user_id}/{conversation_id}")
async def get_conversation_messages(user_id: str, conversation_id: str):
    """Get messages for a specific conversation (lazy loaded on demand)"""
    try:
        from utils.utils import get_firestore_db
        ...
        app_id_for_firestore = "linas-ai-bot-backend"
        conv_ref = db.collection("artifacts").document(app_id_for_firestore).collection("users").document(user_id).collection(config.FIRESTORE_CONVERSATIONS_COLLECTION).document(conversation_id)
