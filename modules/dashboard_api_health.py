...
def tenant_runtime_config_readiness_check() -> dict[str, Any]:
    ...
    tenant env for HA maintenance only — not a product clinic restore
    """
    ...
    reachable = bool(db_health.get("reachable"))
    return {
        "ok": reachable,
        "backend": "postgres",
        "db_reachable": reachable,
        "per_tenant_file_migrate": "skipped",
    }
...
async def ready() -> Any:
    ...
    # BOC booking (not in SaaS) — default OFF; healthy without token or booking IDs.
    ...
    try:
        trc = tenant_runtime_config_readiness_check()
        checks["tenant_runtime_config"] = trc
        if not trc.get("ok"):
            overall_ok = False
    except Exception as e:
