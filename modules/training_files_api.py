...
async def list_training_files() -> Any:
...
async def get_training_file(file_id: str) -> Any:
    """Get content of a specific training file"""
    try:
        if file_id not in TRAINING_FILES:
            return {"success": False, "error": f"Unknown file type: {file_id}"}
...
async def update_training_file(file_id: str, request: dict) -> Any:
...
async def list_training_file_backups(file_id: str) -> Any:
    ...
    try:
        ...
        backups.sort(key=lambda x: str(x.get("created") or ""), reverse=True)

        return {"success": True, "file_id": file_id, "backups": backups, "total": len(backups)}
    except Exception as e:
...
async def restore_training_file_backup(file_id: str, request: dict) -> Any:
...
async def get_training_file_stats(file_id: str) -> Any:
