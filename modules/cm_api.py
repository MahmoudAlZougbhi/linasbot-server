...
from services.ai_setup.request_rule_limits import RequestRuleLimitError
...
async def cm_put_draft(
    ...
    except RequestRuleLimitError as exc:
        if edit_op:
            release_edit(tenant_id=tenant_id, operation_id=edit_op)
        raise HTTPException(
            status_code=400,
            detail={"code": exc.code, "message": str(exc), "request_type": exc.request_type, "count": exc.count},
        ) from exc
    except UnknownSectionError as exc:
        if edit_op:
            release_edit(tenant_id=tenant_id, operation_id=edit_op)
