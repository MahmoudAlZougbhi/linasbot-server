...
def test_polluted_dynamic_message_is_not_sent_as_hello(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Use this rule only" not in (decision.text or "")
    assert "Arabic-specific rule" not in (decision.text or "")
    assert (decision.text or "") != get_dynamic_message(BRAIN_TEMPORARY_ERROR_MESSAGE_KEY, "en")
