...
from services.brain.greeting_detect import inactivity_threshold
from services.brain.greeting_eligibility import evaluate_greeting
...
def test_greeting_uses_existing_12h_window(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "services.brain.greeting_detect.GREETING_INACTIVITY_SECONDS",
        43200,
    )
...
def test_greeting_session_start(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "services.brain.greeting_eligibility.load_dynamic_messages",
...
def test_greeting_only_does_not_prepend_canned_line(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    envelope = FinalReplyEnvelope(
        decision="reply",
        messages=[OutboundMessage(destination="dm", text="How can I help you today?")],
    )
    out = _apply_greeting(turn, "Hi", "instagram_dm", envelope)
    assert [item.text for item in out.messages] == ["How can I help you today?"]
...
async def test_greeting_only_uses_identity_not_agent_retrieve(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Lina's Laser" not in out.envelope.messages[0].text
