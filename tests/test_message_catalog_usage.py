...
def test_paid_plan_draft_does_not_change_public_until_publish() -> None:
    ...
    assert public_lite["intended_price_usd"] == 10.0
    with pytest.raises(CatalogPublishBlocked):
        publish(actor="owner")
    assert current_catalog()["published"] is False
    assert effective_offer_fields("lite")["included_messages"] == 600
...
def test_iap_grant_stays_off_without_cutover_or_pack(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert skipped == {"granted": False, "reason": "pack_unmapped"}
    ...
    assert unmapped == {"granted": False, "reason": "pack_unmapped"}
...
def test_google_notification_attaches_cutover_off_grant(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["message_grant"]["reason"] == "pack_unmapped"
...
async def test_copilot_read_usage_keeps_credits_off_message_remaining(
    ...
    assert result.ok is True
    assert result.data["wallet_unit"] == "messages"
    assert result.data["message_billing_active"] is True
    assert result.data["included_messages"] == 550
    assert result.data["available_messages"] == 550
    assert "messages" in result.data["speak_as"].lower()
