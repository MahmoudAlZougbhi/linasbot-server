...
def test_known_ledger_tenants_union_memory_and_sql(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_known_tenant_ids",
        lambda _s: ["sql-led"],
    )
...
def test_snapshot_includes_memory_lots_when_sql_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_snapshot",
        lambda _s, tid: LedgerSnapshot(tenant_id=tid),
    )
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_list_reservations",
...
def test_settle_uses_memory_hold_when_sql_misses(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr("services.billing.membership.message_ledger_pg.pg_settle", _missing)
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_snapshot",
        lambda _s, tid: LedgerSnapshot(tenant_id=tid),
    )
...
def test_reverse_uses_memory_hold_when_sql_misses(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr("services.billing.membership.message_ledger_pg.pg_reverse", _missing)
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_snapshot",
        lambda _s, tid: LedgerSnapshot(tenant_id=tid),
    )
...
def test_reserve_uses_memory_lot_when_sql_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_reserve",
...
def test_expire_included_also_clears_memory_lots(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("LINAS_MESSAGE_STORE", raising=False)
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr("services.billing.membership.message_ledger_pg.pg_expire_included_before", lambda *_a, **_k: 0)
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_snapshot",
        lambda _s, tid: LedgerSnapshot(tenant_id=tid),
    )
...
def test_list_stale_reserved_unions_memory_when_sql_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    from services.billing.membership import message_ledger as ledger
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_list_stale_reserved",
        lambda *_a, **_k: [],
    )
...
def test_revoke_purchased_also_clears_memory_lots(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("services.billing.membership.message_ledger._pg_session", _sql_session())
    monkeypatch.setattr(
        "services.billing.membership.message_ledger_pg.pg_revoke_purchased",
