...
def test_cache_rebuild_skips_file_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "services.scale.ha_tenant_config_peer_sync.tenant_runtime_config_postgres_required",
        lambda: False,
    )
...
def test_cache_rebuild_calls_rebuild(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "services.scale.ha_tenant_config_peer_sync.rebuild_tenant_cache",
        lambda tenant_id: {"drafts": 2, "tenant": tenant_id},
    )
    monkeypatch.setattr(
        "services.scale.ha_tenant_config_peer_sync.read_published_pointer",
        lambda _tenant: None,
    )
    result = run_tenant_config_cache_rebuild(tenant_id="linas")
