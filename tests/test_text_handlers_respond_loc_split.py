...
from services.brain.inbound.text_handlers_respond import _handle_published_cm_runtime, _process_and_respond
...
def test_text_handlers_respond_modules_under_500_lines() -> None:
    root = Path("services/brain/inbound")
...
def test_text_handlers_respond_preserves_public_api() -> None:
    ...
    assert (
        "from services.brain.inbound.text_handlers_respond import" in parent_src
        or "text_handlers_respond" in parent_src
    )
