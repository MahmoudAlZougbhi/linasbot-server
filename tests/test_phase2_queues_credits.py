...
async def test_creative_handler_refunds_only_via_worker_dlq_path(tmp_path, monkeypatch) -> None:
    """Cancelled Creative jobs raise PermanentJobError; credits stay reserved until worker refunds."""
    ...
    with pytest.raises(PermanentJobError, match="creative_studio_cancelled"):
...
@pytest.mark.asyncio
async def test_creative_image_handler_is_cancelled(tmp_path, monkeypatch) -> None:
    from services.queues.handlers import PermanentJobError, handle_creative_expensive
