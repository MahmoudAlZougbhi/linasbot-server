...
async def test_clear_toggles_after_disconnect_forces_dm_off(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "services.integrations.channel_capability_disconnect.canonical_channel_bindings",
        lambda *_a, **_k: [],
    )
    monkeypatch.setattr(
        "services.integrations.channel_capability_disconnect.dm_capability_state",
        lambda *_a, **_k: {"requested_enabled": True},
    )
    monkeypatch.setattr(
        "services.integrations.channel_capability_disconnect.comment_capability_state",
        lambda *_a, **_k: {"requested_enabled": True},
    )
    monkeypatch.setattr("services.integrations.channel_capability_disconnect.set_channel_toggle", _set)
...
async def test_clear_invalid_dm_when_disconnected(monkeypatch) -> None:
    ...
    from services.integrations.channel_capability_disconnect import clear_invalid_dm_enabled_state_async
...
async def test_clear_invalid_comments_when_disconnected(monkeypatch) -> None:
    ...
    from services.integrations.channel_capability_toggles import clear_invalid_comments_enabled_state_async
