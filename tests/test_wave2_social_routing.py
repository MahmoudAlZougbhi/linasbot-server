...
def test_personal_care_not_human():
    ...
    out = route_social_contact_request(
        "personal care tips", {"channel": "instagram"}, None, "en"
    )
    assert out is None
...
def test_arabic_human_detected():
    ...
    out = route_social_contact_request(
        "بدي احكي مع حدا", {"channel": "instagram"}, None, "ar"
    )
...
def test_force_intent_alone_blocked():
    out = route_social_contact_request(
        "thanks", {"channel": "instagram"}, None, "en", force_intent="booking"
    )
    assert out is None
...
def test_explicit_booking_english():
    out = route_social_contact_request(
        "I want to book an appointment", {"channel": "facebook"}, None, "en"
    )
    assert out is not None
    assert out.intent == "booking"
...
def test_tattoo_beirut_only_number():
    out = route_social_contact_request(
        "tattoo removal", {"channel": "instagram"}, None, "en"
    )
    assert out is not None
...
def test_greetings_stay_on_ai():
    for text in ["Hello", "مرحبا", "kifak", "Au revoir", "ok"]:
        assert (
            route_social_contact_request(text, {"channel": "instagram"}, None, "en")
            is None
        )
...
def test_price_and_prep_stay_on_ai():
    for text in [
        ...
        assert (
            route_social_contact_request(text, {"channel": "instagram"}, None, "ar")
            is None
        )
