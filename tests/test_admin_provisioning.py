...
from services.team.provisioning_service import (
    ProvisionResult,
    audit_line,
...
def test_provision_refuses_when_users_exist() -> None:
    with patch("services.team.provisioning_service.count_existing_users", return_value=1):
...
def test_provision_creates_when_empty() -> None:
    ...
    with (
        patch("services.team.provisioning_service.count_existing_users", return_value=0),
        patch("services.team.provisioning_service.user_service.create_user", return_value=created) as create,
    ):
        result = provision_first_admin(
            email="Owner@Example.com",
            password="SecurePassPhrase99!",
...
def test_provision_platform_owner_sets_platform_tenant() -> None:
    ...
    with (
        patch(
            "services.team.provisioning_service.user_service.get_user_by_email",
            return_value=None,
        ),
        patch(
            "services.team.provisioning_service.user_service.create_user",
