...
def check_conversations() -> None:
    ...
    for user_doc in users_docs:
        ...
        for conv_doc in conversations_docs:
            ...
            # Check messages
            if not messages:
                print("      ❌ NO MESSAGES (will be skipped)")
                stats["no_messages"] += 1
                continue
            ...
            if hours_ago <= 6:
                print(" ✅ (within 6-hour window)")
                stats["within_6h"] += 1
            else:
                print(" ⚠️  (WILL NOT SHOW - older than 6 hours)")
            ...
            if human_takeover:
                if operator_id:
                    ...
                    print("      Human Takeover: YES, NO OPERATOR ⚠️  (in waiting queue only)")
                    stats["waiting_human"] += 1
            else:
                print("      Human Takeover: NO (bot handling)")
                stats["bot_only"] += 1
            ...
            should_appear = (
                ...
                and hours_ago <= 6
                and not (human_takeover and operator_id is None)  # Not waiting for human
            )
            ...
            if should_appear:
                ...
                if human_takeover and operator_id is None:
                    print("         Reason: Waiting in queue (will appear in waiting queue section)")
    ...
    if stats["resolved"] > 0:
        ...
        print("   These won't show in Live Chat (only in Chat History)")
        print("   To make them appear, change status to 'active'")

    if stats["archived"] > 0:
        print(f"\n2. You have {stats['archived']} ARCHIVED conversations")
        print("   These won't show anywhere")
        print("   To make them appear, change status to 'active'")

    if stats["older_than_6h"] > 0:
        print(f"\n3. You have {stats['older_than_6h']} conversations OLDER than 6 hours")
        print("   These are auto-filtered from Live Chat")
        print("   Send a NEW message to create a fresh conversation")
        print("   OR increase ACTIVE_TIME_WINDOW in services/live_chat_service.py")

    if stats["waiting_human"] > 0:
        print(f"\n4. You have {stats['waiting_human']} conversations WAITING FOR HUMAN")
        print("   These appear in the 'Waiting Queue' section, NOT active chats")
        print("   Assign an operator to move them to active chats")

    if total_conversations == 0:
        print("\n⚠️  NO CONVERSATIONS FOUND!")
        print("   Possible causes:")
        print("   1. Backend webhook isn't receiving messages")
        print("   2. Phone number extraction is failing")
        print("   3. save_conversation_message_to_firestore() is returning early")
